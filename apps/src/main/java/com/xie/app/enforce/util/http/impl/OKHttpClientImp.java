...
public class OKHttpClientImp implements IHttpClient {
    ...
    private OKHttpClientImp() {
        ...
        mOkHttpClient = new OkHttpClient().newBuilder()
                .connectTimeout(15, java.util.concurrent.TimeUnit.SECONDS)//设置超时时间
                .readTimeout(15, java.util.concurrent.TimeUnit.SECONDS)//设置读取超时时间
                .writeTimeout(15, java.util.concurrent.TimeUnit.SECONDS)//设置写入超时时间
                .build();
        // 获取主线程的Handler
    ...
    public void get(IRequest request, boolean forceCache, RequestCallBack callBack) {
        ...
        execute(mOkHttpClient.newCall(OkRequest),callBack);
    }
    ...
    private void execute(Call call, final RequestCallBack callBack) {
        call.enqueue(new Callback() {
            @Override
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                if (e instanceof java.net.SocketException) {
                    setFailureCall(0, callBack);
                } else {
